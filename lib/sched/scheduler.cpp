...
{
	//Looks at the next task to be assigned, but why does the
	//current task decide where to start in the queue?
	unsigned nTask = m_nCurrent < MAX_TASKS ? m_nCurrent : 0;
	unsigned nTicks = CTimer::Get ()->GetClockTicks ();
	unsigned max_priority;
	unsigned curr_priority;
	//Find max priority
	...
	{	
		...
		curr_priority = pTask->GetTaskPriority();
		...
		case TaskStateReady:
			...
			else if(max_priority < curr_priority)
				max_priority = curr_priority;
			continue;
		...
		case TaskStateSleeping:
			//If the task won't wake up before the scheduler has to return the next task
			//skip it
			if ((int) (pTask->GetWakeTicks () - nTicks) > 0)
			{
				continue;
	...
	//Iterates through queue of tasks?
	for (unsigned i = 1; i <= m_nTasks; i++)
	{
		if (++nTask >= m_nTasks)
		{
		...
		//what is m_pTask v m_nTasks
		// m_nTasks just # of Tasks in queue as opposed to list of pointers to tasks?
		CTask *pTask = m_pTask[nTask];
		curr_priority = pTask->GetTaskPriority();

		//if the pointer in the nTask index of m_pTask points to 0(nothing?)
		...
		//if the task is suspended skip it
		if (pTask->IsSuspended ())
		{
			continue;
		}

		//if the task is ready, return it
		//MODIFY THIS TO LOOK AT MAX PRIORITY
